...
Let $\hat\kappa^{\star}$ such that $\hat\eta_{\hat\kappa^*} = \textup{max}(\hat\eta)$ and $\kappa^{\star}$ such that $\eta_{\kappa^*} = \textup{max}(\eta)$. Then $\hat\eta_{\hat\kappa^{\star}}$ is a biased estimate of $$\eta_{\kappa^{\star}}$. In short, conditional on $\hat\kappa^{\star}$ being the best estimated pool, the probability is high that it happened by chance and that the real effect is below what we estimated (and this probability increases with the number of pools).
